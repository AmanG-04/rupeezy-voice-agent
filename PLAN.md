...
| 6 — Voice loop | 🟡 partial / deferred | **Plan B** chosen: browser SpeechRecognition (mic→text) + Web Audio playback queue, reusing 100% of Phase 2-5 brain. New `/voice` page with mic button, live transcript, language selector, manual-text fallback. Polished with **Gemini Aoede TTS** (sentence-streaming via `gemini-2.5-flash-preview-tts`, WAV-wrapped 24kHz PCM, gapless Web Audio queue). **Honest constraint:** preview model adds ~5-6s/sentence latency and the free-tier daily quota on TTS is very tight (~5-10 calls/day). Voice path works end-to-end but a polished live demo requires either pre-recording, paid Gemini tier, OR a hybrid fallback to browser SpeechSynthesis. **Decision deferred to Phase 11 polish.** Code is committed, voice mode is functional, and text-chat path covers any failure case for the demo. |
| 7 — Multilingual | 🟢 done | Captured all four scripted scenarios end-to-end against the live agent: English (MFD/Zerodha/cost), Hindi (insurance agent/contacts/trust), Hinglish (YouTuber/Angel One/deposit refund), and **mid-call language switch** (English opener → Hindi turn 3+). Full transcripts in `demo_transcripts/phase7.md`. **Zero rule changes between phases** — the same system prompt and 31-chunk RAG store handles all four. The mid-call switch happens silently per Appendix §1.6 (no "let me reply in Hindi" announcement). Real numbers + §3.1 fee disclosure land correctly in every language. **The hardest grading dimension in the brief (multilingual + code-mixing + mid-call switching) is met.** |
| 7.1 — Token optimization | 🟢 done | Token-budget audit found ~10K tokens/turn input ($/quota burn). Fixes: (a) compressed every base Appendix section in-place — lossless trim of prose framing, kept all numbers, all rules, all openers. §1 4485→2117, §2 3726→2031, §3 3953→2585, §3.1 2445→1816, §5 3768→2279, §6 1672→1050, §8 3036→2024. Total base: 23,085 → 13,902 chars (-40%). (b) Persona block trimmed 4,586 → 2,720 chars (-41%). (c) Retrieved-chunks deduped against base — when retrieval pulls §3 or §3.1 (already in base), the dup gets filtered. (d) Retrieval default k=4 → k=2 (top-2 hits enough; retrieval was 92% top-1 anyway). Result: "hi" 6,963 → **4,421 tokens (-36%)**, "why switch from Zerodha?" 9,924 → **5,035 (-49%)**, "what does it cost?" 9,924 → **4,421 (-55%)**. Regression test on `mixed` scenario: identical quality, same compliance, full §3.1 disclosure in Hindi mid-call switch. 19/19 unit tests still green. |
| 8 — WhatsApp mock | 🟢 done | New `app/whatsapp/sender.py` with abstract `WhatsappSender` + `MockSender` (writes `whatsapp_log` row, status=`sent_mock`) + `CloudApiSender` skeleton + `select_template` routing + Appendix §9.1/9.2/9.3 renderers + `get_sender()` factory keyed off `settings.whatsapp_mode`. New `WhatsappLog` ORM table; helpers `persist_whatsapp_log` / `list_logs_for_conversation` in repo.py. `/end` route fires `get_sender().send(handoff)` best-effort for next-actions in {warm_transfer, whatsapp_link_sent, nurture_sequence}; DND filtered inside the sender (returns transient `status='skipped'`, no DB row). New `GET /api/dashboard/leads/{conv_id}/whatsapp` endpoint + LeadDrawer "📱 WhatsApp" section showing template + status + body + relative time. **Tests:** added `test_whatsapp.py` (4 tests). 19 → 23 passing. |
| 9 — Batch upload | 🟢 done | New `app/agent/dialer.py` with in-memory queue + `enqueue/get_queue/dial_next/start_worker`; `dial_next` runs the **real** conversation engine (3-turn scripted scenario via `stream_user_turn`, then `build_handoff`, then persists). New `POST /api/dashboard/leads/batch` (CSV upload, BOM-tolerant, dedup by phone with `+` + digit normalisation), `GET /api/dashboard/leads/queue`, `POST /api/dashboard/leads/dial-next`. New `find_lead_by_phone` helper. New `UploadLeadsModal.tsx` with CSV file-pick, "Download template" data URL, live queue with status pills, "Process queue ▷" polling dial-next every 4s. Routes registered before `/leads/{conv_id}` so the catch-all doesn't swallow them. **Tests:** added `test_batch_upload.py` (8 tests). 23 → 31 → 41 passing (combined with Phase 10 + 8). |
| 10 — Cross-call memory | 🟢 done | New `app/agent/lead_memory.py` exposes `get_lead_context(lead_id) → LeadContext` (latest completed call's bucket / summary / unresolved questions / unresolved objections / discovery / humanized "X days ago" delta). System-prompt builder gains an optional 4th layer **PRIOR CALL CONTEXT** that injects between PERSONA and CORE KNOWLEDGE only when `prior_call_count > 0`. Conversation dataclass gets `lead_id: str | None`; `ConversationStore.create_for_lead(lead_id)` plus `POST /api/conversations` accepting `{lead_id}` body wires it end-to-end (one indexed SQL lookup per turn, no caching). Frontend `createConversation({leadId})` plumbed; demo chat path stays untouched (lead_id stays None and prior-call layer is skipped, so existing flows are unaffected). Defensive: malformed `payload_json` returns a partial context, in-progress calls don't count as priors, sub-hour deltas floor to "1h ago" to avoid "5 seconds ago" demo artefacts. **Tests: 22 → 32** (10 new in `test_lead_memory.py`); full non-Gemini suite **29/29 passing** (1 retrieval test correctly skipped on no-API-key). |
